...
namespace SS_OpenCV
{
    public class FFT
    {
        ...
        /// Get direct FFT 
        /// </summary>
        /// <param name="img">Image BGR</param>
        /// <param name="fft_Amp">Amplitude</param>
        /// <param name="fft_Phase">Phase</param>
        public static void GetFFTAmpAndPhase(Image<Bgr, byte> img, out Image<Gray, float> fft_Amp, out Image<Gray, float> fft_Phase)
        {
        ...
        /// Get direct FFT 
        /// </summary>
        /// <param name="img">Image GRAY</param>
        /// <param name="fft_Amp">Amplitude</param>
        /// <param name="fft_Phase">Phase</param>
        public static void GetFFTAmpAndPhase(Image<Gray, float> imgSynt, out Image<Gray, float> fft_Amp, out Image<Gray, float> fft_Phase)
        {
            IntPtr imageFFT;
            Size s = GetFFTReAndIm(imgSynt, out imageFFT);
        ...
        /// <param name="fft_Re"></param>
        /// <param name="fft_Im"></param>
        public static void GetFFTReAndIm(Image<Bgr, byte> img, out Image<Gray, float> fft_Re, out Image<Gray, float> fft_Im)
        {
            // prepare image with 2 channels for DFT
            IntPtr imageFFT;
        ...
        /// <returns></returns>
        public static Image<Gray, byte> GetFFT_InverseAmpAndPhase(Image<Gray, float> fft_Amp, Image<Gray, float> fft_Phase)
        {
            //rearange the fft output
        ...
        /// <param name="fft_Im"></param>
        /// <returns></returns>
        public static Image<Gray, byte> GetFFT_InverseReAndIm(Image<Gray, float> fft_Re, Image<Gray, float> fft_Im)
        {
            //merge the 2 channels into one image 
        ...
        /// <returns></returns>
        public static Image<Gray, float> GetFFT_InverseReAndIm(IntPtr imageFFT, Size s)
        {
            // calculate DFT 
            CvInvoke.cvDFT(imageFFT, imageFFT, Emgu.CV.CvEnum.CV_DXT.CV_DXT_INVERSE, 0);
        ...
        /// <returns></returns>
        public static Image<Gray, float> PrepareForVizualization(Image<Gray, float> fft, bool showLog)
        {
        ...
        /// <returns></returns>
        public static Image<Gray, float> GenerateFilterMask(Size size, bool isHighPass, int width)
        {
        ...
        /// <param name="width"></param>
        /// <returns></returns>
        public static Image<Gray, float> GenerateGaussian(Size size, bool isHighPass, double width)
        {
            Image<Gray, float> mask = new Image<Gray, float>(size);
            ...
            for (int u = 0; u < size.Height; u++)
            {
                for (int v = 0; v < size.Width; v++)
                {
                    ...
                    if (isHighPass)
                        mask[u, v] = new Gray(1 - 
1.0 * Math.Exp(-Math.Sqrt(uu * uu + vv * vv) / (double)(2 * width * width)));
                    else
        ...
        /// <summary>
        /// Generate artificial image of cos
        /// </summary>
        /// <param name="i"></param>
        /// <returns></returns>
        public static Image<Gray, float> GenerateSyntethicImage(float i)
        {
            Image<Gray, float> result = new Image<Gray, float>(400, 400);

            float cosV;
            for (int x = 0; x < result.Width; x++)
            {
                cosV = (float)Math.Cos(i * x) * 255;

                for (int y = 0; y < result.Height; y++)
                {
                    result.Data[y, x, 0] = cosV;
                }
            }
            return result;
        }
